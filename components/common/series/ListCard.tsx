...
import EditListNamePopup from "../movies/EditListNamePopup";
...
function ListCard({ list, onView, onDelete }: Props) {
    ...
    list.media_type === "tv"
      ...
      : list.item_count === 1
        ...
        const onTogglePublic = async(target:boolean) => {
          try {
            await axiosPrivate.patch(`/series/lists/update/${list.id}`,{isPublic: target ? 1 : 0})
            setIsPublic(target)
        ...
        const onSave = async(name:string) => {
          try {
            await axiosPrivate.patch(`/series/lists/update/${list.id}`,{name})
  ...
  const handleDelete = async () => {
    setDeleting(true)
    try {
      await axiosPrivate.delete(`/series/lists/delete/${list.id}`)
    ...
    <div className="group relative flex flex-col gap-4 rounded-xl border border-black/10 bg-black/2 p-4 transition duration-200 hover:border-black/20 hover:shadow-lg hover:shadow-black/5 dark:border-white/10 dark:bg-white/2 dark:hover:border-white/20 dark:hover:shadow-black/30">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={() =>
            onView ? onView() : router.push(`/series/lists/${list.id}`)
          }
